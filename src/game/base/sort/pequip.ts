...
import { getIncreAffnumPdf } from "@/game/gacha/reliq";
import { toCDF } from "@/game/gacha/utils";

export interface ICharEqiup {
    [slotKey: string]: Artifact;
}

export type ISlotKey = keyof ICharEqiup;
...
export abstract class PEquipSort {
    sort(
        arts: Artifact[],
        allArts: Artifact[],
        builds: IBuild[],
        ...
        for (let art of arts) {
            ...
            equip.forEach((charEquip, charKey) => {
                ...
                if (
                    ignoreIndividual &&
                    !gameUtils.isInSetGroup(build.set, art.set)
                )
                    return;
                const artCur = Math.round(
                        gameUtils.calcAffnumCur(art, build.weight) * 10,
                    ),
                    otherArtCur = Math.round(
                        gameUtils.calcAffnumCur(otherArt, build.weight) * 10,
                    );
